...
private:
  ...
  //---- photons
  std::vector<float> _std_vector_photonGen_pt;
...
GenDumper::GenDumper(const edm::ParameterSet& iConfig)
{
  ...
  myTree_ -> Branch("std_vector_photonGen_pt", "std::vector<float>", &_std_vector_photonGen_pt);
...
void GenDumper::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (_debug) std::cout << " analyze " << std::endl;
  ...
  if (_doLHE) iEvent.getByToken(_productLHET,productLHEHandle);
  
  lhef::HEPEUP LHEhepeup;
  if (_doLHE) LHEhepeup = (*(productLHEHandle.product())).hepeup();
...
//   std::cout << " Size genmet = " << genMET->size() << std::endl;
  ...
  //---- photons
  _std_vector_photonGen_pt.clear();  
  for (reco::GenParticleCollection::const_iterator genPart = ptOrderedGenParticles.begin(); genPart != ptOrderedGenParticles.end(); genPart++){
    int id = abs(genPart->pdgId());
    if (id == 22 && genPart->status()==1) { //---- photon
      _std_vector_photonGen_pt.push_back( genPart->pt() );
    }
  }
  ...
  _std_vector_AGen_pt.clear();
  _std_vector_AGen_mass.clear();
  for (reco::GenParticleCollection::const_iterator genPart = ptOrderedGenParticles.begin(); genPart != ptOrderedGenParticles.end(); genPart++){
    int id = abs(genPart->pdgId());
    if (id == 25) { //---- h/a
      _std_vector_AGen_pt.push_back( genPart->pt() );
      _std_vector_AGen_mass.push_back( genPart->mass() );
    }
  }
